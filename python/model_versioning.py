...
class ModelRegistry:
    ...
    def log_model(
        ...
        with mlflow.start_run(run_name=f"{self.model_name}_v{version}") as run:
            ...
            # Log model
            mlflow.pytorch.log_model(model, "model", registered_model_name=self.model_name)
    ...
    def load_model(
        self, version: Optional[str] = None, stage: Optional[str] = None
    ) -> torch.nn.Module:
        """
    ...
    def promote_model(self, version: str, stage: str = "Production"):
        ...
        client.transition_model_version_stage(name=self.model_name, version=version, stage=stage)
