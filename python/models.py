...
class AnalyzeResponse(BaseModel):
    ...
    user_id: int = Field(..., description="User ID that was analyzed")
    score: str = Field(..., description="Fraud probability score (0.000 to 1.000)")
    is_fraud: bool = Field(..., description="Whether user is flagged as fraudulent")
...
class GraphLink(BaseModel):
    ...
    is_laundering: bool = Field(
        ..., description="Whether transaction is part of laundering pattern"
    )
...
class HealthResponse(BaseModel):
    ...
    cache: Optional[Dict[str, Any]] = Field(default=None, description="Cache health and statistics")
