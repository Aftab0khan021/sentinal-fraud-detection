...
class AdvancedExplainer:
    ...
    def explain_gnn(self, node_idx):
        ...
        explanation = self.gnn_explainer(self.data.x, self.data.edge_index, index=node_idx)
        ...
        if edge_mask is not None:
            ...
            for idx in top_indices:
                ...
                important_edges.append({"source": src, "target": dst, "importance": float(score)})
        ...
        if node_mask is not None:
            ...
            for idx in top_features:
                feature_importance.append(
                    {"feature_index": int(idx), "importance": float(target_mask[idx].item())}
                )
...
def init_explainer_module(
    model_path="models/best_fraud_detector_improved.pt", graph_path="data/graph_enhanced.pkl"
):
