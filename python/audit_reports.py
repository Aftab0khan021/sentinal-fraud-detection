                        ...
                        try:
                            ...
                            if self.verify_signature(entry):
                                ...
                                results["errors"].append(
                                    f"Tampered entry in {log_file.name}: {entry.get('timestamp')}"
                                )
...
def generate_report(days: int = 1):
    ...
    print(
        f"Fraud Detected:     {stats['fraud_detected']} ({(stats['fraud_detected']/stats['analyses_run']*100 if stats['analyses_run'] else 0):.1f}%)"
    )
